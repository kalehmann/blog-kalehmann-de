...
description:  >-
  ...
  Delving into the different scan code sets of PS/2 keyboards and programming
  a keyboard to send keystrokes in scan code set 1.
lang:             en
last_modified_at: 2026-08-23 16:45:19 +0200
layout:           post
tags:
  - Coding
  - OsDev
...
[![ThinkPad T480 with Dvorak Keyboard][1]][1]
...
<details>
  ...
  This post explains in detail why switching GRUB terminal input to AT mode and
...
[blog posts][4] and [Stack Overflow answers][5].
...
I found neither official nor unofficial documentation about the binary format
used by GRUB for keymaps.
However, taking a look at the [source code of GRUB's `keymap` command][6]
...
* Finally comes the keyboard mapping as four arrays with 160 four-byte entries,
  each little endian `uint32_t`.
  ```c
...
As a workaround, [*ckbcomp* from the AUR][13] can be used.
Alternatively, since *ckbcomp* is just a Perl script it can be extracted from
the package without installing it on the system:
...
GRUB supports [different input sources][14].
...
For example, `serial` for serial keyboards or `at_keyboard` for the IBM
AT-compatible 8042 PS/2 controller emulation.
...
from the keyboard, which can then be translated by GRUB.
...
but could not perform any actions - for example selecting another entry - inside
GRUB.

First, I reverted all my changes, restarted the ThinkPad and entered the GRUB
...
keyboard.
It turned out that `terminal_input at_keyboard` was responsible for the
degradation right away.
After some digging through the internet I found a [relevant bug report][16] in
the Debian bug tracker explaining what is going on.
The keyboard sends data to the computer indicating which action was performed
on it - namely which keys were pressed and released.
This data is called a *"scan code"*.
...
This PC introduced the XT keyboard which sent keystrokes encoded in what is
nowadays known as *scan code set 1*.
Most common keys have single byte keystrokes with the most significant bit
set if the key is pressed and cleared when the key is released.
...
The following table lists a selection of scan codes and their translation
to sets 1 and 2.
...
The PS/2 controller uses `0x60` as its data port (where the keyboard data can be
read).
...
Multiple places on the internet mention setting the input method with
...
The I/O ports `0x64` and `0x60` are both used for communication with the PS/2
controller.
The first I/O port (`0x64`) maps to the PS/2 controller's command/status register
and the second I/O port (`0x60`) maps to the PS/2 controller's data register.
...
the next byte from the data port into offset 0 of the controller's RAM.
...
| 3   | ?                                            |
| 4   | First port clock disabled                    |
| 5   | Second port clock disabled                   |
| 6   | XLAT (translation of set 2 to set 1) enabled |
| 7   | ?                                            |

This byte sent to the controller has the 2nd, 5th, and 6th bits set -
...
I need and also disables the second port clock?
I am not sure about the third part, some sources list the 5th bit as enabling
the keyboard in general, which would make more sense I guess.
Since I do not exactly know the meaning of the 5th bit, I will leave it cleared.

1. 1st attempt: I use `terminal_input at_keyboard console` instead.
...
2. 2nd attempt: I tried running the commands one after another
   ```
...
Up until now, I only talked to the PS/2 controller and tricked it into
translating the scan codes from set 2 to set 1.
...
The following arguments are available for the command:
...
After digging around a bit in the source code of the
...
This parameter is used by the following code - which is exactly what I need.
```c
/*
...
3. Load the keymap inside GRUB and switch the keyboard to scan code set 1.
   ...
   ```
   at the end.
4. Force the [Linux kernel to reset the keyboard](#linux-reset-at-keyboard) during startup.
...
5. Recreate the GRUB configuration.
  ...
  [1]: {{ "assets/2021-08-grub-dvorak/thinkpad_t480_dvorak.jpg" | relative_url }}
